 ...
 * @version 2.0
 *  
 * <p> This project is the 2.0 version or iteration of BigBox Application version 1.0. This is a console application and consists of Division and Store classes stored
 * into their respective packages. It implements the DAO Factory Pattern and stores and gets the data from array list that is stored into StoreArray.java class file.StoreArray.java includes and stores the data in the form of array list .</p>
...
public class BigBoxApp2a {

	/** The Constant NAME_SIZE. */
	private static final int NAME_SIZE = 0;
	
	/** The id. */
	private static int fID = 0;
	
	/** The Store DAO. */
	private static StoreArray StoreDAO = null;
	...
	private static void displayAllStores() {
		...
		for (int i = 0; i < stores.size(); i++) {
			s = stores.get(i);
			sb.append(padWithSpaces("[Store:  store#=" + s.getStoreNumber() + ", Division Number="
			...
			sb.append("[Store:  store#=" + s.getStoreNumber() + ", Division Number=" + s.getDivisionNumber()
					+ ", Sales=" + s.getFormattedSales() + "]\n" + "[Facilty: " + "id=" + s.getId() + ", name="
					+ s.getName() + ", address=" + s.getAddress() + ", city=" + s.getCity() + ", state=" + s.getState()
					+ ", zip=" + s.getZipCode() + "]");
			sb.append("\n");
		}
	 ...
	 * @return the string
	 */
	public static String padWithSpaces(String s) {
